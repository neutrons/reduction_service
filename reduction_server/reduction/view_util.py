...
from reduction_server.remote import view_util  
import importlib
import logging
...
def get_latest_job(request, reduction_process):
    ...
    if len(latest_jobs)>0:
        ...
        job_info = view_util.query_remote_job(request, latest_job.remote_id)
...
def import_module_from_app(instrument_name_lowercase, module_name):
    ...
    """
    module_str = "reduction_server.%s.%s"%(instrument_name_lowercase,module_name)
