...
def get_new_batch_url(run=None, ipts=None):
    if run is None:
        return reverse('reduction.views.reduction_configuration', args={'instrument_name': 'eqsans' })
    return reverse('reduction.views.reduction_configuration',args={'instrument_name': 'eqsans' })+"?reduction_name=Reduction for %s&experiment=%s&data_file=%s" % (run, ipts, run)
